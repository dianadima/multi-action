...
if isfield(cfg,'axis'), xaxis = cfg.axis; else, xaxis = 1:ncat; end
...
for istim = 1:ncat
    ...
    if sc
        scatter(jittermat(:)+xaxis(istim)*ones(nobs,1), data(istim,:), 'MarkerEdgeColor','w','MarkerFaceColor',cl,'SizeData',ms);
    else
        scatter(xaxis(istim)*ones(nobs,1), data(istim,:), 'MarkerEdgeColor','w','MarkerFaceColor',cl,'SizeData',ms);
    end

end

h=boxplot(data', 'Positions',xaxis,'Colors','k','Symbol','');
set(h,{'linew'},{1.2});
xticklabels(labels)
xtickangle(90)
box off
