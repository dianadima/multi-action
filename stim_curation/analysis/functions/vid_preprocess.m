function [coord, startframe, frame1] = vid_preprocess(videofile, newfilename, nframes, targetsize, varargin)
...
if vid.FrameRate<19, warning('Low framerate: %s', videofile); end
...
else %3 second videos at set fps
    nframes = fps*3; 
    startframe = 1;
    if vidsize(4)<nframes, nframes = vidsize(4); newvid.FrameRate = vid.FrameRate; %true for 3 videos
    else, newvid.FrameRate = fps; % set this to same across videos 
    end
end
...
for f = startframe:startframe+nframes-1
    ...
    if f==startframe, frame1 = frame; end
